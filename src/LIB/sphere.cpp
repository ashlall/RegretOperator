...
point_set_t* construct_I(int dim, int k)
{
  ...
  // The best radius of I is 2 * \sqrt(d) theoretically,but in practice,
  // other radius also works well in some cases, e.g., \sqrt(d)
  if (k < dim * dim)
  {
  ...
  // Case 3 of Sphere
  // t is the number of hyperplanes in each family that are used
  // to divide the facet of hypercube (t >= 0)
  ...
  for (int i = 0; i < dim; i++)
  {
    for (int j = 0; j < output.size(); j++)
    {
      ...
      // put the resulting point in I, remenber to scale the point
      // so that it lies on the sphere of radius
...
point_set_t* sphereWSImpLP(point_set_t* point_set, int k)
{
  ...
  if (k >= 2 * dim)
  {
    // cout << "BEFORE STEP 2"<< endl;
    // Step 2: Construct set I
    point_set_t* I;
    I = construct_I(dim, k - dim);

    // cout << "BEFORE STEP 3" << endl;
  ...
  // cout << "FLAG1: compute mrr_0(p)"<< endl;
  ...
  lastRound_max = max;
  // cout << "FLAG3: Before the while loop count < k" << endl;
  while (count < k)
  {
    // Find a point with maximum regret
    max = NULL;
    worst = 0;
    // cout << "FLAG4: : During the while loop" << endl;

    for (int i = 0; i < N; i++)
    {
      // cout <<i << "FLAG4: : During the for loop" << endl;
      // upper bounding
      // cout <<"FLAG6: error in the dot_prod"<<  product << endl;
      // double product = dot_prod(sub(point_set->points[i], lastRound_max), directions->points[i]);
      // cout <<"ID:" << point_set->points[i]->id << endl;
      // cout <<"max"<< lastRound_max->id << endl;
      if (worst > rr[i])
      {
      ...
      {
        // cout << "FLAG5: problems with the else if " << endl;
        // solve the exact LP
        rr[i] = worstDirection(count, result, point_set->points[i], directions->points[i]->coord);
        directions->points[i]->id = -1;
      }

      // cout << "FLAG2: update the current worst point"<< endl;
  ...
  delete[] active;
  delete[] b_value;
  // cout << "BEFORE release_point_set"<< endl;
