...
int main(int argc, char** argv)
{
  ...
  FILE* log;
	...
	if (argc < 3)
	{
		printf ("usage: ./vary-D file-prefix K\n");
		return 0;
	}
  ...
  N= 10000;
	sprintf(filename, "plot/varyD-%s-N%d-k%d", argv[1],N, K);
	fp = fopen(filename, "w");

	sprintf(filename, "plot/varyD-%s-N%d-k%d.log", argv[1],N, K);
  log = fopen(filename, "w");
    ...
    M = db/10.0;
      ...
      maxIndex = new int[K];
			sprintf(filename, "Data/%s%d-%d-%d.dat", argv[1], D, N, j + 1);
      cout << filename << endl;
	...
	times[i/2] += (end.tv_sec - start.tv_sec) + (double) (end.tv_usec - start.tv_usec) /1000000;
			...
			printf("%s,D =%d, db=%d,M = %lf, %lf s, %lf\n", methods[i],D, db, M, times[i/2]/(j+1), rr[i]/(j+1));
			fprintf(log, "%s,D =%d, db=%d, M = %lf, %lf s, %lf\n", methods[i],D, db, M, times[i/2]/(j+1), rr[i]/(j+1));
      ...
      printf("%s,D =%d, db=%d, M = %lf,  %lf\n", methods[i],D, db, M,  rr[i]/(j+1));
      fprintf(log, "%s,D =%d, db=%d, M = %lf, %lf\n", methods[i],D, db, M, rr[i]/(j+1));
