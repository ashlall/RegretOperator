...
*/
double evaluateRandom(int D, int N, int K, point_set_t *p, point_set_t * S, int REPEATS, int VERBOSE)
{
	int i, j;
   ...
   // Evaluate regret using lots of random utility functions
	for(i = 0; i < REPEATS; ++i)
	{
		...
		maxK = 0.0;
		for(j = 0; j < K; ++j)
			if (dot_prod(S->points[j], sample) > maxK)
				maxK = dot_prod(S->points[j], sample);


		maxN = 0.0;
        for(j = 0; j < N; ++j)
			if (dot_prod(p->points[j], sample) > maxN)
				maxN = dot_prod(p->points[j], sample);
